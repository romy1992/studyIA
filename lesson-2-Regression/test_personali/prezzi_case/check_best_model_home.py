...
pipeline = Pipeline([
    ...
    ('model', LinearRegression())  # Placeholder per i modelli
])
...
grid_search = GridSearchCV(
    ...
    scoring='neg_mean_squared_error',  # Valutazione basata sul Mean Squared Error negativo
    verbose=2,
    n_jobs=-1  # Usa tutti i core disponibili
)

# Adatta la GridSearch sui dati di addestramento
...
lasso_model = grid_search.best_estimator_.named_steps['model']
feature_importance = lasso_model.coef_

# Associa i coefficienti ai nomi delle feature
feature_names = dataset.iloc[:, :-1].columns  # Assicurati di avere i nomi delle colonne
